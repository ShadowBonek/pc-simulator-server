...
import { nvmeModel } from "./nvmeModel";
...
export const getComponents: RequestHandler = async (req: any, res) => {
  ...
  try {
    const components = await nvmeModel.findAndCountAll({
      where: {
        [Op.or]: [
